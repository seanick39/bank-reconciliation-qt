...
namespace brlib::parse
{
    ...
    public:
        ...
        explicit DebitCreditError(errorType et):
            std::invalid_argument(
              et == errorType::eq_zero ? "Both debit and credit can't be zero" : "Both debit and credit can't be non-zero.") {}
    };

    class ColNumberError : public std::runtime_error
    {
    public:
        explicit ColNumberError(const str& s):
            std::runtime_error(s) {}
    };
    ...
    {
    public:
        explicit InvalidHeaderError(const str& s):
    ...
    {
    public:
        explicit DateParseError(const str& e):
