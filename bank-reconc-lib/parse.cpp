 ...
 * and leave it there. */
     ...
     * iterating over formats in dateFormats.*/
                ...
                !dateFormatRetrieved)
            {
                ...
                if (dtPos)
                { // bad hack; normally we'd want to substr from delimPos + 1
                    iss.str(line.substr(dtPos + 1));
                }
                else
                { // but if date is first char, we can't add 1;
   ...
   * param. */
        auto pos = [&](long cnt) {
            return getDelimPos(s, cnt, options.delimChar);
        };
